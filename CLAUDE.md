...
- **Valoración integral (2026-09-25, 3.22.0, pedida por el usuario para no depender de la IA en las validaciones
  ...
  **3.31.0 (2026-09-26, lista de pedidos del usuario)**: «Análisis» es la SEGUNDA pestaña (Resultado · Análisis ·
  Reporte; se mueven el botón y el panel tras `tarjetaResultadosHtml`). Al final de «Análisis» va la sensibilidad al costo
  de instalación de Conductor económico («¿Puede el costo de instalación cambiar la decisión?», mismo diseño y clases
  `ce-inst-*`, con «Alternativa» en vez de «Opción»): motor `sensibilidadInstalacion(r, escenarios)` (solo si
  `criterioRecomendado === "costo"`, frente a la RECOMENDADA y solo contra las que también cumplen; el valor por km usa la
  longitud de la recomendada) y `pesoConductoresKgKm(esc)` (3 × N × masa, promediado por longitud) para la pista de peso;
  `evaluarEscenario` suma ahora `costoConductores`. También en el reporte de texto, el PDF (`instalacionHtml(m, {doc})`) y
  la hoja «Análisis» del Excel. «Exportar» suma «Documento de Word (.docx)»: `crearDocxDocumento(doc, {titulo, apaisado})`
  en `js/ai/docx.js` convierte el MISMO `documentoPdf()` (colspan → `gridSpan`, celdas `vi-doc-bueno/neutral/malo` con los
  rellenos del Excel, hoja horizontal con más de 3 alternativas; verificado abriéndolo en Word por COM). Fc en PDF/Excel/
  reporte con `num(v, 2, 4)` (2 decimales, más si los tiene). Subtítulo «Costos de conductor e instalación». La «i» del
  costo de instalación ya no empieza por «Opcional:» y cierra con «Si el campo se deja vacío o en cero, solo se considera
  el costo del conductor en el análisis.». «Nueva valoración» justificada a la derecha (`.vi-trabajando` es flex; el texto
  va en su propio `<span>`). «Duplicar»/«Quitar» de la alternativa sin negrilla y el icono `copy` con trazo 1.6 SOLO ahí
  (CSS sobre el `<g>`); en >480px «Quitar» 5 px (≈20 %) más lejos del chevron. La conclusión en una línea mide 40 px, igual
  que «Exportar» (`min-height` y relleno vertical de 8 px). Descripciones: Valoración integral «Evalúa conductores con
  todos los cálculos técnico-económicos.», Configuración de IA «Clave de API, modelo y datos guardados en este
  navegador.» y Cálculos (Inicio) «Ampacidad, pérdidas, regulación, ocupación de ductos y más.».
  **3.32.0 (2026-09-26, pedido del usuario)**: con la tensión GENERAL, la corriente de falla y el tiempo de despeje van
  ...
  suministro del conductor). Si el campo se deja vacío o en cero, solo se considera el costo del conductor en el
  análisis.» (texto del usuario).
...
- **«Plegar tarjeta» al pie (2026-09-26, 3.34.0, propuesta del usuario)**: `activarPlegables` agrega a CADA tarjeta
  plegable un enlace sutil (`.btn-plegar-pie`: gris, 0.8rem, sin negrilla ni recuadro; color del tema al pasar el cursor)
  al final de su cuerpo (`.plegable-pie`), visible SOLO si la tarjeta abierta es más alta que la ventana menos la barra
  de la app (`revisarPie`, exportada; un solo ResizeObserver + `resize` de la ventana, nada continuo). En Valoración
  integral va a la izquierda de la línea de «Agregar tramo» (`.vi-tramo-pie`). Al pulsarlo pliega con animación y, si la
  barra de título quedó fuera de la vista, desplaza la página para dejarla bajo la barra de la app (suave solo si
  `plegado.animar`). Oculto no deja espacio (`:has`). Pruebas: «plegar desde abajo» en `verify_tarjetas_plegables.html`.
- **Transiciones al aparecer contenido (2026-09-26, 3.33.0, pedido del usuario: «Guardadas» aparecía de golpe)**:
  `js/util/revelar.js`, misma animación de las tarjetas plegables (altura 0 → natural + opacidad, 220 ms; lo de abajo se
  desliza). `revelar(el, {resaltar})` para lo recién insertado (con `resaltar`, el borde brilla con `--accent` y se apaga
  en 1.2 s: tarjetas NUEVAS de alternativa, tramo, opción y tipo, solo al pulsar «Agregar»/«Duplicar», no al restaurar);
  `mostrar(el, visible)` cambia `hidden` con animación (al ocultar, `hidden` se pone al terminar; deshabilitar antes los
  campos). Las filas de tabla (`tr`) solo se desvanecen. `activarDetallesAnimados()` (una vez, en `app.js`) anima TODOS
  los `<details>` al pulsar el resumen, salvo `.menu-mas` y `.sin-animar`. Aplicado a: «Guardadas» y la caja «Guardar»,
  alternativas/tramos, campos de «Por alternativa» (solo al pulsar la casilla), tarjetas nuevas de Pérdidas, Regulación,
  Conductor económico y Ocupación, campos de «Catálogo» (Ocupación), «Calcular con la posición del sol» y tarjeta del GCC
  (solo con un evento de la persona: `e.isTrusted`; al restaurar, al instante), historial de Perfil → Catálogos y la
  fila nueva de Codificación. No se anima un solo campo que se habilita (decisión: demasiado movimiento). Sin animación con
  `prefers-reduced-motion`, y en los arneses de `tools/` (`revelado.animar = null` → `!window.__BASE_PATH__`); se prueba
  en `verify_tarjetas_plegables.html` («transiciones al aparecer contenido») con `animar = true` y avanzando a mano.
...
- **Fase 2 (hecha 2026-09-24, versión 3.19.0)**. Decisiones del usuario (elegidas entre opciones): cada «Guardar» se
  **publica de inmediato** (con confirmación), **Eliminar borra** el registro (se recupera desde el historial), y se
  ...
  - **Codificación de entregables (2026-09-26, 3.32.0)**: sexto catálogo del servidor (`codificacion` en
    `CATALOGOS_EDITABLES`, en las reglas —HAY QUE PUBLICARLAS en la consola de Firebase— y en `backend-mock.js`). Como la
    tabla no abre ficha, se edita EN LA FILA (decidido con el usuario, sin ventana emergente): para el administrador, dos
    columnas «Editar» | «Eliminar» al final (botones de icono); Editar vuelve casillas las tres celdas y los botones pasan
    a Guardar (✓, icono `check` nuevo) y Cancelar (X); Enter guarda y Esc cancela; «Agregar registro» (al final de la
    `.toolbar`) pone una fila vacía arriba; la especialidad sugiere las existentes (`datalist`). Validación
    (`validarCodificacion`, exportada): los tres campos obligatorios y el código sin repetir (sin distinguir mayúsculas).
    Cada cambio pide confirmación y se publica de inmediato (`guardarCatalogo`, con la misma protección de conflictos);
    el historial y «Volver a esta versión» son los de Perfil → Catálogos. `render(container, params, {confirmar})`. La
    vista ahora escapa todo (antes metía el JSON en `innerHTML` sin escapar). Pruebas: sección «codificación de
    entregables» de `verify_catalogos.html`.
  - Perfil (3.32.0, pedido del usuario): los subtítulos de Calculadoras (`.pf-form .pf-grupo`) usan el estilo de los de
    «Parámetros avanzados» de Valoración integral (barrita, texto y línea hasta el borde; sin la línea superior de
    antes). Las tablas de Usuarios (`.pf-usuarios`), Datos y Catálogos (`.pf-datos`) llevan el texto más suave del
    historial de IA (`color-mix` 82 %), la primera columna en peso 400 (con Segoe UI el 500 se ve casi como negrilla) y
    los botones en 600; las insignias de rol no cambian.
