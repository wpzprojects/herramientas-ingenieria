...
## Funciones con IA (Gemini, OpenAI o Anthropic)
...
- **Proveedor**: se elige de forma global en *Configuración* (tarjeta «Proveedor de IA»): Google (Gemini), OpenAI (ChatGPT) o Anthropic (Claude). Solo Gemini admite guardar la clave en el servidor (clave «personal»/«compartida», ver más abajo); OpenAI y Anthropic son siempre BYOK local (clave propia guardada en este navegador). Cada conversación guardada recuerda con qué proveedor se creó.
...
- **Clave de API**: cada usuario pega la suya (gratuita en el caso de Gemini; de pago según el plan de la cuenta en OpenAI/Anthropic) en *Configuración*. Se guarda solo en su navegador (`localStorage`, o `sessionStorage` si no elige recordarla) y solo se envía al proveedor elegido. No hay backend. El modelo no está fijo en el código: se lista con la clave y se elige en Configuración.
- **`js/ai/gemini.js`, `js/ai/openai.js`, `js/ai/anthropic.js`**: un cliente REST por `fetch` por proveedor, con la misma interfaz pública (`generar`, `listarModelos`, `elegirModeloPorDefecto`, `probarConexion`); `js/ai/proveedores.js` es el registro central que resuelve cuál usar. El formato de conversación interno (el que ya usaba Gemini) es neutro: cada cliente traduce hacia/desde su propio formato de mensajes y de *tool calling*. El service worker ignora orígenes externos, así que no cachea estas llamadas.
- **`js/ai/tools.js` + `analisis.js`**: la IA **no calcula**. Hay 16 herramientas: 10 en el agente estándar (7 calculadoras —incluida `calcular_conductor_economico`— + `barrer_parametro`, `buscar_conductor`, `buscar_tuberia`) y 6 opcionales que solo usa un agente que las active (`dimensionar_conductor`, `verificar_conductor`, `resolver_valor_limite` de diseño, `guardar_ficha_proyecto` para registrar datos confirmados sin calcular, y `convertir_unidades`/`convertir_coordenadas` de Varios); el proveedor de IA decide cuál usar (function calling) y la app las ejecuta con los motores de `js/calc/`. Cada herramienta replica lo que hace la vista antes de llamar al motor (búsqueda del conductor en el catálogo, valores por defecto, primera coincidencia del catálogo) y valida rangos; lo que no viene del usuario se registra como *supuesto*. Las de Varios devuelven al modelo 12 cifras significativas (`cifras`), no 6: con 6 un Este de 4 881 143 m quedaría con metros de error. Hay un tope de rondas y de cálculos por pregunta (Configuración). Desde 2026-09-19 las herramientas están alineadas con las pantallas rediseñadas (varios tramos, dato de partida en MW/MVA/A, conductores por fase, clasificación Óptimo/Aceptable/Elevado, corriente de falla a soportar, varios tipos de conductor y radio de curvatura, corriente circulante en la pantalla, unidades del catálogo completo con calibre AWG/kcmil y coordenadas con ~500 códigos EPSG); ver la tabla de `docs/ia-herramientas.md`.
...
- **Agentes de análisis** (`js/ai/agentes-analisis.js`, botón «Agentes» del Asistente técnico): cada agente es el prompt de sistema con el que trabaja el proveedor de IA + las instrucciones del reporte + una temperatura opcional + la lista de herramientas que puede usar (casillas por grupo: Calculadoras, Catálogos, Análisis, Varios; `catalogoHerramientas()` en `tools.js`; el bloqueo lo aplica el código, no el prompt). Hay **dos predefinidos** (`AGENTES_PREDETERMINADOS`, ambos de solo lectura, viven en el código): el **estándar** (`SISTEMA_ANALISIS`/`PROMPT_REPORTE` de `analisis.js`, calcula rápido) y el **riguroso** (`SISTEMA_RIGUROSO`/`PROMPT_REPORTE_RIGUROSO`, pide todos los datos por categoría antes de calcular y arma una memoria de cálculo formal; usa las 16 herramientas). Ninguno se guarda ni se puede borrar; se pueden ver y duplicar. Los propios (duplicados o nuevos) se editan y se guardan en `localStorage`. A los propios la app les agrega siempre al final `REGLA_FIJA` (la IA no calcula: todo número sale de una herramienta).
...
- **Verificación**: `tools/verify_ia.html` (arnés en el navegador, sin clave ni internet, los tres proveedores simulados con mocks: contrasta cada adaptador contra fórmulas independientes y los motores) y `tools/preview_ia.html` (vista previa de las pantallas con Gemini simulado). Ver instrucciones en el encabezado de cada archivo.
...
Sin iniciar sesión (o con un correo que no esté en la lista) la app funciona como **visitante**: solo cinco módulos (Ocupación de ductos, Pérdidas, Regulación, Conductores desnudos y Distancias de seguridad); el resto se ve pero sin enlace, y Varios y Funciones con IA quedan bloqueadas. Con un correo autorizado (**usuario**) se habilitan todos los módulos, y el **administrador** además gestiona la lista de usuarios (Perfil) y la clave compartida de Gemini en el servidor (Funciones con IA → Configuración). Sin internet, el acceso completo dura 15 días desde la última vez que el servidor lo confirmó (`js/auth/acceso.js`; se renueva solo al abrir con conexión). Reglas por ruta en `js/auth/permisos.js`; pruebas en `tools/verify_acceso.html`. Es un control de uso de la interfaz, no de confidencialidad (los archivos del sitio son públicos).

## Acceso con Google y Firebase (menú lateral → Perfil)

Pantalla de acceso restringido (`#/perfil`, «Perfil y configuración avanzada»: ítem con icono de usuario al fondo del menú lateral, encima del botón de contraer; la ruta anterior `#/ayuda/configuracion` redirige ahí). Pide iniciar sesión con Google y solo deja entrar a los correos de una **lista guardada en el servidor** (no en este repositorio, que es público). Dentro se gestionan los usuarios y el color del tema; la clave de Gemini en el servidor (personal/compartida) se guarda en **Funciones con IA → Configuración** (tarjeta «Origen de la clave», solo con Gemini activo). Requiere internet; el resto de la app sigue funcionando sin conexión.
...
- **Valoración integral** (`#/calculos/valoracion-integral`, 2026-09-25, solo usuarios): evalúa de 1 a 6 escenarios de conductor para una misma conexión (potencia, FP, Fc y longitud COMUNES; cada escenario con su tensión, red aérea o subterránea, conductor y conductores por fase) con todos los criterios a la vez: ampacidad (IEEE 738 en aérea, IEC 60287 en subterránea), pérdidas, regulación, cortocircuito (veredicto solo si se indica la corriente de falla) y costo total actualizado (si hay precio de la energía y costo del conductor). Límites = las referencias de diseño de siempre (pérdidas 3 %, regulación 10 %, corriente ≤ ampacidad). Recomendado = el de menor costo total entre los que cumplen todo. Lo que casi no cambia (ambiente, terreno, disposición de fases, cable subterráneo) va plegado en «Parámetros avanzados». No hay fórmulas nuevas: `js/calc/valoracion-integral.js` combina los motores sin tocarlos. Supuestos: varios conductores por fase en aérea = haz; en subterránea = N ternas en paralelo en el mismo banco (ampacidad con calentamiento mutuo × N, impedancia / N), cables monopolares en trébol. Cada escenario admite hasta 4 tramos en serie (p. ej. aéreo + subterráneo), cada uno con su conductor y su longitud: pérdidas, caída y costos se suman; en ampacidad manda el tramo más cargado y en cortocircuito el más débil. Botón «Exportar» a PDF o Excel (.xlsx sin librerías, `js/util/xlsx.js`). «Guardar» y «Guardadas»: las valoraciones se guardan con un nombre en el dispositivo y, con sesión e internet, también en la cuenta (Firestore `usuarios/{correo}/valoraciones`, `js/util/valoraciones-guardadas.js`; publicar las reglas de `firebase/firestore.rules`). Pruebas: `tools/verify_valoracion_integral.html`.
