...
export async function render(container) {
  ...
  aplicarDefectos(container, "perdidas"); // valores por defecto personales (Perfil > Calculadoras), antes de restaurar lo escrito
  ...
  function crearTramo(id) {
    ...
    return {
      ...
      /** Aplica una foto de `bruto()`, disparando los "change" en cascada (red -> material -> calibre -> referencia) para que las listas se repueblen antes de fijar el valor final. */
      aplicarBruto: (d) => {
        if (!d) return;
        selRed.value = d.red;
